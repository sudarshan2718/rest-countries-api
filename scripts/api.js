...
async function fetchAllCountries() {
    ...
    let url = "https://restcountries.com/v2/all?fields=name,capital,flag,population,region,subregion,currencies,languages,borders,topLevelDomain"
    await fetch(url)
...
async function fetchByregion(country_region) {
    let countryData = null;
    let url = `https://restcountries.com/v2/continent/${country_region.toLowerCase()}?fields=name,capital,flag,population,region,subregion,currencies,languages,borders,topLevelDomain`
    await fetch(url)
        .then(function (response) {
            return response.json()
        }).then(data => {
...
async function fetchByName(country_name) {
    let countryData = null;
    let url = `https://restcountries.com/v2/name/${country_name.toLowerCase()}?fields=name,capital,flag,population,region;`;
    await fetch(url)
        .then(function (response) {
            return response.json()
        }).then(data => {
...
async function fetchByFullName(country_name) {
    let countryData = null;
    let url = `https://restcountries.com/v2/name/${country_name.toLowerCase()}?fullText=true&fields=name,capital,flag,population,region,subregion,currencies,languages,borders,topLevelDomain`;
    await fetch(url)
        .then(function (response) {
            return response.json()
        }).then(data => {
